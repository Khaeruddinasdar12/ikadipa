...
class BeritaController extends Controller
{
    ...
    {
        $data = Berita::select('beritas.id', 'beritas.judul', 'beritas.isi','beritas.gambar', 'beritas.created_at', DB::raw('group_concat(concat(kategori_beritas.nama)SEPARATOR ", ") as kategori'))
    ...
    public function kategoriBerita(Request $request)
    {
        ...
        ->select('beritas.id', 'beritas.judul', 'beritas.gambar', 'beritas.created_at', 'beritas.isi', DB::raw('group_concat(concat(kategori_beritas.nama)SEPARATOR ", ") as kategori'))
        ->leftjoin("kategori_beritas",\DB::raw("FIND_IN_SET(kategori_beritas.id,beritas.kategori)"),">",\DB::raw("'0'"))
        ->get();
        if($data == '') {
            return response()->json([
